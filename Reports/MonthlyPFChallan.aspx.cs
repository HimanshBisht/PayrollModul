...
public partial class SalaryModule_MonthlyPFChallan : System.Web.UI.Page
{
    string constr = "";
    SqlCommand cmd;
    SqlDataAdapter da;
    Hashtable hash;
    decimal GrandTotalPF = 0;
    decimal GrandTotalBasicPlusDA = 0;
    ...
    public void Employee(int? EmployeeID)
    {
        try
        {
            ...
            cmd.Parameters.AddWithValue("@PFDeductNew", PFDeduct.Yes);
            ...
            ddlemployee.Items.Insert(0, new ListItem("All PF Employees", "0"));
            con.Close();
        }
        catch (Exception ex)
        {
    ...
    public void Bindgrid()
    {
        ...
        {
            SqlConnection con = new SqlConnection(constr);
            cmd = new SqlCommand("PFChallenReport", con);
            cmd.Parameters.AddWithValue("@MonthID", ddlMonth.SelectedValue);
            cmd.Parameters.AddWithValue("@YearID", ddlYear.SelectedValue);
            ...
            if (dt.Rows.Count > 0)
            {
                GrandTotalPF = dt.AsEnumerable().Sum(row => row.Field<decimal>("PF"));
                GrandTotalBasicPlusDA = dt.AsEnumerable().Sum(row => row.Field<decimal>("DAForReport"));
                lblSTMT.Text = "K.R.Mangalam World School, Faridabad " + ddlMonth.SelectedItem.Text + " - " + ddlYear.SelectedItem.Text + "<br />" + "Staff Provident Fund Statement";
    ...
    private void ExportGridToExcel()
    {
        try
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=PF_Report_" + ddlMonth.SelectedItem.Text.Substring(0, 3) + " - " + ddlYear.SelectedItem.Text + "_FDB.xls");
            Response.Charset = "";
    ...
    protected void grdrecord_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                //decimal lblEPFWages = Convert.ToDecimal((Label)e.Row.FindControl("lblEPFWages"));

                string b = e.Row.Cells[6].Text;
                string c = e.Row.Cells[7].Text;
                decimal d = Convert.ToDecimal(b) - Convert.ToDecimal(c);
                e.Row.Cells[8].Text = d.ToString();
                //e.Row.Cells[11].Text= b.ToString();
            }
            if (e.Row.RowType == DataControlRowType.Footer)
            {
                // Label lblGrandTotalPF = (Label)e.Row.FindControl("lblGrandTotalPF");
                /// Label lblGrandDAForReport = (Label)e.Row.FindControl("lblGrandDAForReport");
                // lblGrandTotalPF.Text = GrandTotalPF.ToString();
                // lblGrandDAForReport.Text = GrandTotalBasicPlusDA.ToString();
    ...
    protected void btnExport_Click(object sender, EventArgs e)
    {
        ...
        Response.AddHeader("content-disposition", string.Format("attachment; filename={0}", "ECRChallan.txt"));
        Response.ContentType = "application/text";
        StringBuilder str = new StringBuilder();
        //for (int i = 0; i < grdrecord.Columns.Count; i++)
        //{
        //    str.Append(grdrecord.Columns[i].HeaderText + ',');
        //}.TrimEnd('#', '~', '#')
        //str.Append("\n");
        string delimiterChars = "#~#";
        for (int j = 0; j < grdrecord.Rows.Count; j++)
        {
            for (int k = 0; k < grdrecord.Columns.Count; k++)
            {
                str.Append((grdrecord.Rows[j].Cells[k].Text + "#~#"));
            }
            str.Remove(str.Length - 3, 3);
            str.Append(Environment.NewLine).ToString();
        }
        Response.Write(str.ToString());
        Response.End();
    }

    protected void textfile()
    {
        ...
        foreach (GridViewRow row in grdrecord.Rows)
        {
            ...
            txtFile.Remove(txtFile.Length - 3, 3);
            txtFile += "\r\n";
        }
