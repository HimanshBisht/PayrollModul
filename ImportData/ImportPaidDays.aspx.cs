...
public partial class SalaryModule_ImportPaidDays : System.Web.UI.Page
{
    string constr = "";
    SqlCommand cmd;
    SqlDataAdapter da;
    ...
    private void Import_To_Grid(string FilePath, string Extension)
    {
        try
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                hash = new Hashtable();
                hash = (Hashtable)Session["User"];
                string PaidDays = dt.Rows[i]["Paid Days"].ToString();
                SqlConnection con = new SqlConnection(constr);
                cmd = new SqlCommand("SaveImportPaidDays", con);
                cmd.CommandType = CommandType.StoredProcedure;
    ...
    private void ExportGridToExcel()
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                Response.AddHeader("content-disposition", string.Format("attachment; filename={0}", "Fill_Paid_Days_" + ddlMonth.SelectedItem.Text.Substring(0, 3) + "_" + ddlYear.SelectedItem.Text + "_FDB.xls"));
                Response.ContentType = "application/ms-excel";
                string str = string.Empty;
                int Count = 0;
                foreach (DataColumn dtcol in dt.Columns)
                {
                    if (dtcol.ColumnName == "ProfileID" || dtcol.ColumnName == "EmployeeID" || dtcol.ColumnName == "Emp_Code" || dtcol.ColumnName == "SystemNumber" || dtcol.ColumnName == "AssignEmpCode" || dtcol.ColumnName == "Name" || dtcol.ColumnName == "Designation")
                    {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    {
                        if (j == 0 || j == 1 || j == 2 || j == 3 || j == 4 || j == 5 || j == 8)
                        {
                    ...
                    {
                        Response.Write(str + ds.Tables[3].Rows[0]["PaidDays"].ToString());
                    }
                    str = "\t";
                    Response.Write("\n");
                    Row++;
    ...
    protected void btnSaveTemplate_Click(object sender, EventArgs e)
    {
        try
        {
            if (flImportPaidDays.HasFile)
            {
                string FileName = ddlMonth.SelectedValue + ddlYear.SelectedItem.Text + "_" + Path.GetFileName(flImportPaidDays.PostedFile.FileName);
                string Extension = Path.GetExtension(flImportPaidDays.PostedFile.FileName);
                string FolderPath = ConfigurationManager.AppSettings["FolderPath"];

                string FilePath = Server.MapPath(FolderPath + FileName);
                flImportPaidDays.SaveAs(FilePath);
