...
public partial class SalaryModule_SalaryApproval : System.Web.UI.Page
{
    string constr = "";
    SqlCommand cmd;
    SqlDataAdapter da;
    ...
    protected void btnApproved_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            cmd.Parameters.AddWithValue("@Type", "SalaryApproved");
            con.Open();
            ...
            if (Count > 0)
            {
                ScriptManager.RegisterClientScriptBlock(Page, this.GetType(), "validate", "javascript: alert('Salary Approved by " + Convert.ToString(hash["Name"].ToString()) + "');", true);
            }
            else
            {
                ScriptManager.RegisterClientScriptBlock(Page, this.GetType(), "validate", "javascript: alert('Salary Already Approved For this Month and Year.');", true);
            }
        }
        catch (Exception ex)
        {
    ...
    protected void btnNotApproved_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            cmd.Parameters.AddWithValue("@ModeChangeRemarks", null);
            cmd.Parameters.AddWithValue("@User", Convert.ToString(hash["Name"].ToString()));
            cmd.Parameters.AddWithValue("@Type", "SalaryNotApproved");
            con.Open();
            int Count = cmd.ExecuteNonQuery();
            con.Close();
            ...
            if (Count > 0)
            {
                ScriptManager.RegisterClientScriptBlock(Page, this.GetType(), "validate", "javascript: alert('Salary Not Approved by " + Convert.ToString(hash["Name"].ToString()) + "');", true);
            }
            else
            {
                ScriptManager.RegisterClientScriptBlock(Page, this.GetType(), "validate", "javascript: alert('Salary Already Not Approved For this Month and Year.');", true);
            }
        }
        catch (Exception ex)
        {
