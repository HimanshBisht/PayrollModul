...
public partial class SalaryModule_CalculateRecovery : System.Web.UI.Page
{
    string constr = "";
    SqlCommand cmd;
    SqlDataAdapter da;
    ...
    public void Month()
    {
        try
        {
            ...
            ddlRecoveryMonth.DataSource = dt;
            ddlRecoveryMonth.DataTextField = "MonthName";
            ddlRecoveryMonth.DataValueField = "MonthID";
            ddlRecoveryMonth.DataBind();
            ddlRecoveryMonth.Items.Insert(0, new ListItem("Select Month", "0"));
            con.Close();
        }
        catch (Exception ex)
        {
    ...
    public void Year()
    {
        try
        {
            ...
            ddlRecoveryYear.DataSource = dt;
            ddlRecoveryYear.DataTextField = "Year";
            ddlRecoveryYear.DataValueField = "YearID";
            ddlRecoveryYear.DataBind();
            ddlRecoveryYear.Items.Insert(0, new ListItem("Select Year", "0"));
            con.Close();
        }
        catch (Exception ex)
        {
    ...
    public void Bindgrid()
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                lblCalculatedText.Text = "Change Respective Column Value to Calculate Salary For Recovery";
            }
            else
            {
    ...
    public void Clear()
    {
        try
        {
            ...
            pnlDetails.Visible = false;
            lblActualGrossTotalSalary.Text = "0.00";
            lblRecoveryGrossTotalSalary.Text = "0.00";
            lblFinalRecoveryAmount.Text = "0.00";
            lblTotalRecoveryAmount.Text = "0.00";
            ddlRecoveryMonth.ClearSelection();
            ddlRecoveryYear.ClearSelection();
            ...
            lblCalculatedText.Text = string.Empty;
            lblActualSalaryText.Text = string.Empty;
            lblPreRecoveryAmount.Text = "0.00";
        }
        catch (Exception ex)
        {
            ScriptManager.RegisterClientScriptBlock(Page, this.GetType(), "validate", "javascript: alert('" + ex.Message + "');", true);
    ...
    public void SaveCalculatedSalary()
    {
        try
        {
            hash = new Hashtable();
            hash = (Hashtable)Session["User"];
            decimal RecoveryValue = Convert.ToDecimal(lblFinalRecoveryAmount.Text);
            int SalaryMonthID = Convert.ToInt32(ddlMonth.SelectedValue);
            int SalaryYearID = Convert.ToInt32(ddlYear.SelectedValue);
            int RecoveryMonthID = Convert.ToInt32(ddlRecoveryMonth.SelectedValue);
            int RecoveryYearID = Convert.ToInt32(ddlRecoveryYear.SelectedValue);
            string RecoveryMonth = ddlRecoveryMonth.SelectedItem.Text;
            string RecoveryYear = ddlRecoveryYear.SelectedItem.Text;

            if (RecoveryValue > 0)
            {
                if ((RecoveryMonthID > SalaryMonthID && RecoveryYearID == SalaryYearID) || (RecoveryYearID > SalaryYearID))
                {
                    foreach (GridViewRow row in grdrecord.Rows)
                    {
                        ...
                        SqlConnection con = new SqlConnection(constr);
                        cmd = new SqlCommand("SaveImportDeduction", con);
                        cmd.CommandType = CommandType.StoredProcedure;
                        ...
                        cmd.Parameters.AddWithValue("@MonthID", RecoveryMonthID);
                        cmd.Parameters.AddWithValue("@Month", RecoveryMonth);
                        cmd.Parameters.AddWithValue("@YearID", RecoveryYearID);
                        cmd.Parameters.AddWithValue("@Year", RecoveryYear);
                        cmd.Parameters.AddWithValue("@DeductionRecoveryType", RecoveryType);
                        cmd.Parameters.AddWithValue("@DeductionValue", RecoveryValue);
                ...
                {
                    ScriptManager.RegisterClientScriptBlock(Page, this.GetType(), "validate", "javascript: alert('Failed : Recovery Selected Month and year should be greater than Salary Month and Year.');", true);
                }
            }
            else
            {
                ScriptManager.RegisterClientScriptBlock(Page, this.GetType(), "validate", "javascript: alert('Failed : Recovery Amount should be greater than 0');", true);
            }
    ...
    public void CheckValidSaveMonthAndYear()
    {
        try
        {
            SqlConnection con = new SqlConnection(constr);
            cmd = new SqlCommand("ShowSalaryMaking", con);
            cmd.Parameters.AddWithValue("@MonthID", ddlRecoveryMonth.SelectedValue);
            cmd.Parameters.AddWithValue("@YearID", ddlRecoveryYear.SelectedValue);
            cmd.Parameters.AddWithValue("@ProfileID", ddlemployee.SelectedValue);
            cmd.Parameters.AddWithValue("@SalaryMakeStatus", SalaryStatus.Release);
            cmd.Parameters.AddWithValue("@IsApprove", IsApprove.Approve);
            cmd.CommandType = CommandType.StoredProcedure;
            ...
            if (dt.Rows.Count > 0)
            {
                ScriptManager.RegisterClientScriptBlock(Page, this.GetType(), "validate", "javascript: alert('Failed : Salary is already given for this Month and Year, Please select valid month and year to save Recovery Details.');", true);
            }
            else
            {
    ...
    protected void btnSaveRecovery_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            foreach (GridViewRow row in grdrecord.Rows)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    if (GrossTotal >= TotalDeduction)
                    {
                        ...
                        lblRecoveryGrossTotalSalary.Text = GrossTotalSalaryAfterDeduction.ToString("0.00");
    ...
    public void CalculateRecovery()
    {
        try
        {
            decimal RecoverySalary = Convert.ToDecimal(lblRecoveryGrossTotalSalary.Text);
            decimal ActualSalary = Convert.ToDecimal(lblActualGrossTotalSalary.Text);
            decimal TotalRecoveryAmount = ActualSalary - RecoverySalary;
            decimal OtherAdjustment = 0;
            decimal PreRecoveryValue = Convert.ToDecimal(lblPreRecoveryAmount.Text);

            if (txtOtherAmt.Text.Length > 0 && txtOtherAmt.Text != ".")
            {
                OtherAdjustment = Convert.ToDecimal(txtOtherAmt.Text);
            }

            decimal RecoveryValue = TotalRecoveryAmount + PreRecoveryValue + OtherAdjustment;
            lblTotalRecoveryAmount.Text = TotalRecoveryAmount.ToString("0.00");
            lblFinalRecoveryAmount.Text = RecoveryValue.ToString("0.00");
        }
        catch (Exception ex)
        {
            ScriptManager.RegisterClientScriptBlock(Page, this.GetType(), "validate", "javascript: alert('" + ex.Message + "');", true);
    ...
    public void GetPrint()
    {
        try
        {
            ...
            if (RecoveryValue > 0)
            {
                if (Convert.ToInt32(ddlRecoveryMonth.SelectedValue) > 0 && Convert.ToInt32(ddlRecoveryYear.SelectedValue) > 0)
                {
                    lblCalculatedText.Text = string.Empty;
                    lblCalculatedText.Text = "Recovery Report of " + ddlemployee.SelectedItem.Text + " For the Month of " + ddlRecoveryMonth.SelectedItem.Text + " - " + ddlRecoveryYear.SelectedItem.Text;
    ...
    public void CheckPreValue()
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                lblPreRecoveryAmount.Text = dt.Rows[0]["DeductionValue"].ToString();
    ...
    protected void ddlRecoveryMonth_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            if (Convert.ToInt32(ddlRecoveryMonth.SelectedValue) > 0 && Convert.ToInt32(ddlRecoveryYear.SelectedValue) > 0)
    ...
    protected void ddlRecoveryYear_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            if (Convert.ToInt32(ddlRecoveryMonth.SelectedValue) > 0 && Convert.ToInt32(ddlRecoveryYear.SelectedValue) > 0)
