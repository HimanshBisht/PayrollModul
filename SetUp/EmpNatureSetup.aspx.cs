...
public partial class SalaryModule_EmpNatureSetup : System.Web.UI.Page
{
    string constr = "";
    SqlConnection con;
    SqlCommand cmd;
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            cmd = new SqlCommand("ManageNatureofEmp", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@EmpNature", txtNature.Text);
            cmd.Parameters.AddWithValue("@User", Convert.ToString(hash["Name"].ToString()));

            if (ViewState["NatureID"] == null)
            {
                cmd.Parameters.AddWithValue("@NatureID", 0);
                cmd.Parameters.AddWithValue("@Type", "Save");
            }
            else
            {
                cmd.Parameters.AddWithValue("@NatureID", ViewState["NatureID"]);
            ...
            {
                ScriptManager.RegisterClientScriptBlock(Page, this.GetType(), "validate", "javascript: alert('" + txtNature.Text + " Already Exist.');", true);
            }
    ...
    protected void grdrecord_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if (e.CommandName == "lnkEdit")
            {
                ...
                cmd = new SqlCommand("ManageNatureofEmp", con);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@NatureID", e.CommandArgument);
                cmd.Parameters.AddWithValue("@EmpNature", txtNature.Text);
                cmd.Parameters.AddWithValue("@User", Convert.ToString(hash["Name"].ToString()));
                cmd.Parameters.AddWithValue("@Type", "GetRecords");
                con.Open();
                ...
                da.Fill(dt);
                con.Close();
                txtNature.Text = dt.Rows[0]["EmpNature"].ToString();
                ViewState["NatureID"] = e.CommandArgument;
                btnSave.Text = "Update";
            }
    ...
    private void ExportGridToExcel()
    {
        try
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=Emp Nature Setup Report.xls");
            Response.Charset = "";
