...
public partial class SalaryModule_StaffTypeSetup : System.Web.UI.Page
{
    string constr = "";
    SqlConnection con;
    SqlCommand cmd;
    ...
    public void Clear()
    {
        try
        {
            txtStaffType.Text = string.Empty;
            ViewState["StaffTypeID"] = null;
            btnSave.Text = "Save";
        }
        catch (Exception ex)
        {
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            cmd = new SqlCommand("ManageStaffType", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@StaffType", txtStaffType.Text);
            cmd.Parameters.AddWithValue("@User", Convert.ToString(hash["Name"].ToString()));

            if (ViewState["StaffTypeID"] == null)
            {
                cmd.Parameters.AddWithValue("@StaffTypeID", 0);
                cmd.Parameters.AddWithValue("@Type", "Save");
            }
            else
            {
                cmd.Parameters.AddWithValue("@StaffTypeID", ViewState["StaffTypeID"]);
            ...
            {
                ScriptManager.RegisterClientScriptBlock(Page, this.GetType(), "validate", "javascript: alert('" + txtStaffType.Text + " Already Exist.');", true);
            }
    ...
    protected void grdrecord_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if (e.CommandName == "lnkEdit")
            {
                ...
                cmd = new SqlCommand("ManageStaffType", con);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@StaffTypeID", e.CommandArgument);
                cmd.Parameters.AddWithValue("@StaffType", txtStaffType.Text);
                cmd.Parameters.AddWithValue("@User", Convert.ToString(hash["Name"].ToString()));
                cmd.Parameters.AddWithValue("@Type", "GetRecords");
                con.Open();
                ...
                da.Fill(dt);
                con.Close();
                txtStaffType.Text = dt.Rows[0]["StaffType"].ToString();
                ViewState["StaffTypeID"] = e.CommandArgument;
                btnSave.Text = "Update";
            }
    ...
    private void ExportGridToExcel()
    {
        try
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=Staff Type Setup Report.xls");
            Response.Charset = "";
            ...
            using (StringWriter sw = new StringWriter())
            {
                ...
                {
                    grdrecord.Columns[6].Visible = false;
