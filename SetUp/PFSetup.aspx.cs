...
public partial class SalaryModule_PFSetup : System.Web.UI.Page
{
    string constr = "";
    SqlCommand cmd;
    SqlDataAdapter da;
    ...
    public void Bindgrid()
    {
        ...
        {
            SqlConnection con = new SqlConnection(constr);
            cmd = new SqlCommand("ShowPFSetUp", con);
            cmd.CommandType = CommandType.StoredProcedure;
            con.Open();
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (HasUpdatePF > 0)
            {
                ...
                cmdProfile.Parameters.AddWithValue("@PFDeductNew", PFDeductType.YesWithNewScheme);
                cmdProfile.Parameters.AddWithValue("@PFDeductOld", PFDeductType.YesWithOldScheme);
                ...
                if (dtProfile.Rows.Count > 0)
                {
                    foreach (DataRow row in dtProfile.Rows)
                    {
                        ...
                        if (PFDeduct == "1")
                        {
                            if (GetBasicPlusDAForPF >= PFMaxRange)
                            {

                                PFValue = Converter((PFMaxRange * PF) / 100);

                            }
                            else if (GetBasicPlusDAForPF < PFMaxRange)
                            {

                                PFValue = Converter((GetBasicPlusDAForPF * PF) / 100);
                            }
                        }
                        else if (PFDeduct == "3")
                        {
                        ...
                        SqlCommand cmdSaveProfile = new SqlCommand("SavePFSetUp", con, trans);
                ...
                if (HasUpdatePF > 0 || HasUpdateProfile > 0)
                {
                    trans.Commit();
                    con.Close();
                    Bindgrid();
    ...
    protected void grdrecord_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if (e.CommandName == "lnkEdit")
            {
                ...
                cmd = new SqlCommand("ShowPFSetUp", con);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@PFID", e.CommandArgument);
                con.Open();
                DataTable dt = new DataTable();
                da = new SqlDataAdapter(cmd);
                da.Fill(dt);
                ...
                ViewState["PFID"] = e.CommandArgument;
