...
public partial class SalaryModule_AppointmentSetup : System.Web.UI.Page
{
    string constr = "";
    SqlConnection con;
    SqlCommand cmd;
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            cmd = new SqlCommand("ManageAppointment", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@AppointmentType", txtAppointment.Text);
            cmd.Parameters.AddWithValue("@User", Convert.ToString(hash["Name"].ToString()));

            if (ViewState["AppointmentID"] == null)
            {
                cmd.Parameters.AddWithValue("@AppointmentID", 0);
                cmd.Parameters.AddWithValue("@Type", "Save");
            }
            else
            {
                cmd.Parameters.AddWithValue("@AppointmentID", ViewState["AppointmentID"]);
            ...
            {
                ScriptManager.RegisterClientScriptBlock(Page, this.GetType(), "validate", "javascript: alert('" + txtAppointment.Text + " Already Exist.');", true);
            }
    ...
    protected void grdrecord_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if (e.CommandName == "lnkEdit")
            {
                ...
                cmd = new SqlCommand("ManageAppointment", con);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@AppointmentID", e.CommandArgument);
                cmd.Parameters.AddWithValue("@AppointmentType", txtAppointment.Text);
                cmd.Parameters.AddWithValue("@User", Convert.ToString(hash["Name"].ToString()));
                cmd.Parameters.AddWithValue("@Type", "GetRecords");
                con.Open();
                ...
                da.Fill(dt);
                con.Close();
                txtAppointment.Text = dt.Rows[0]["AppointmentType"].ToString();
                ViewState["AppointmentID"] = e.CommandArgument;
                btnSave.Text = "Update";
            }
    ...
    private void ExportGridToExcel()
    {
        try
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=Appointment Setup Report.xls");
            Response.Charset = "";
