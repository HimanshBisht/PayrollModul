...
public partial class SalaryModule_TDSHeaderwiseRules : System.Web.UI.Page
{
    string constr = "";
    SqlCommand cmd;
    SqlDataAdapter da;
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            cmd = new SqlCommand("ManageHeaderwiseRules", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@RuleText", txtHeadRule.Text);
            ...
            {
                cmd.Parameters.AddWithValue("@RuleID", 0);
                cmd.Parameters.AddWithValue("@Type", "Save");
            }
            else
            {
                cmd.Parameters.AddWithValue("@RuleID", ViewState["RuleID"]);
            ...
            {
                ScriptManager.RegisterClientScriptBlock(Page, this.GetType(), "validate", "javascript: alert('" + txtHeadRule.Text + " Already Exist.');", true);
            }
    ...
    protected void grdrecord_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if (e.CommandName == "lnkEdit")
            {
                ...
                cmd = new SqlCommand("ManageHeaderwiseRules", con);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@RuleID", e.CommandArgument);
                cmd.Parameters.AddWithValue("@RuleText", null);
                cmd.Parameters.AddWithValue("@HeaderID", ddlHeaderList.SelectedValue);
                cmd.Parameters.AddWithValue("@User", Convert.ToString(hash["Name"].ToString()));
                cmd.Parameters.AddWithValue("@Type", "GetRecords");
                ...
                da.Fill(dt);
                con.Close();
                txtHeadRule.Text = dt.Rows[0]["RuleText"].ToString();
                txtMaxAmount.Text = dt.Rows[0]["MaxAmount"].ToString();
                ViewState["RuleID"] = e.CommandArgument;
                btnSave.Text = "Update";
            }
    ...
    private void ExportGridToExcel()
    {
        try
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=" + ddlHeaderList.SelectedItem.Text + "_Rules.xls");
            Response.Charset = "";
