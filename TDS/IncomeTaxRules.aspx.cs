...
public partial class SalaryModule_IncomeTaxRules : System.Web.UI.Page
{
    string constr = "";
    SqlCommand cmd;
    SqlDataAdapter da;
    ...
    public void Bindgrid()
    {
        ...
        {
            SqlConnection con = new SqlConnection(constr);
            cmd = new SqlCommand("ManageIncomeTaxRules", con);
            cmd.Parameters.AddWithValue("@Type", "GetRecords");
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                grdrecord.Caption = "<h3>" + "Income Tax Rules " + ddlYear.SelectedItem.Text + " - " + (Convert.ToInt32(ddlYear.SelectedItem.Text) + 1) + "<h3 />";
    ...
    public void Clear()
    {
        try
        {
            ...
            txtIncomeTaxRates.Text = string.Empty;
            txtEducationCess.Text = string.Empty;
            txtHigherEducationCess.Text = string.Empty;
            ddlSpecialCase.ClearSelection();
            grdrecord.DataSource = null;
            grdrecord.DataBind();
            grdrecord.Caption = string.Empty;
            btnSave.Text = "Save";
            ViewState["IncomeTaxRuleID"] = null;
            pnlButtons.Visible = false;
        }
        catch (Exception ex)
        {
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            //{
                ...
                cmd.Parameters.AddWithValue("@IncomeTaxRates", txtIncomeTaxRates.Text);
                cmd.Parameters.AddWithValue("@EducationCess", txtEducationCess.Text);
                cmd.Parameters.AddWithValue("@HigherEducationCess", txtHigherEducationCess.Text);
                cmd.Parameters.AddWithValue("@SpecialCase", ddlSpecialCase.SelectedValue);
                cmd.Parameters.AddWithValue("@User", Convert.ToString(hash["Name"].ToString()));
                if (ViewState["IncomeTaxRuleID"] == null && btnSave.Text == "Save")
                {
                    cmd.Parameters.AddWithValue("@IncomeTaxRuleID", 0);
                    cmd.Parameters.AddWithValue("@Type", "Save");
                }
                else
                {
                    cmd.Parameters.AddWithValue("@IncomeTaxRuleID", ViewState["IncomeTaxRuleID"]);
    ...
    protected void grdrecord_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if (e.CommandName == "lnkEdit")
            {
                ...
                cmd = new SqlCommand("ManageIncomeTaxRules", con);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@IncomeTaxRuleID", e.CommandArgument);
                cmd.Parameters.AddWithValue("@Type", "GetRecords");
                cmd.Parameters.AddWithValue("@User", null);
                con.Open();
                DataTable dt = new DataTable();
                ...
                txtIncomeTaxRates.Text = dt.Rows[0]["IncomeTaxRates"].ToString();
                txtEducationCess.Text = dt.Rows[0]["EducationCess"].ToString();
                txtHigherEducationCess.Text = dt.Rows[0]["HigherEducationCess"].ToString();
                ddlSpecialCase.SelectedValue = dt.Rows[0]["SpecialCase"].ToString();
                btnSave.Text = "Update";
                ViewState["IncomeTaxRuleID"] = e.CommandArgument;
            }
    ...
    private void ExportGridToExcel()
    {
        try
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=Income_Tax_Rules_" + ddlYear.SelectedItem.Text + "-" + (Convert.ToInt32(ddlYear.SelectedItem.Text) + 1) + ".xls");
            Response.Charset = "";
