...
public partial class SalaryModule_IDFApproval : System.Web.UI.Page
{
    string constr = "";
    SqlCommand cmd;
    SqlDataAdapter da;
    ...
    public void Employee()
    {
        try
        {
            ...
            cmd.Parameters.AddWithValue("@IsApprove", Type.NotApproved);
    ...
    public void Bindgrid()
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                lblSTMT.Text = "K.R.Mangalam World School, Faridabad <br /> List of Pending IDF Approval Employees for " + ddlYear.SelectedItem.Text + "-" +
        ...
        try
        {
            ...
            foreach (GridViewRow row in grdrecord.Rows)
            {
                if (((CheckBox)row.FindControl("SelectChk")).Checked == true)
                {
                    ...
                    cmd.Parameters.AddWithValue("@Type", "ApprovedIDF");
            ...
            {
                Clear();
                ScriptManager.RegisterClientScriptBlock(Page, this.GetType(), "validate", "javascript: alert('Record Approved Sucessfully.');", true);
            }
